...
export default function AdminPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [laddersRes, matchesRes, challengesRes, requestsRes] = await Promise.all([
          fetch("/api/ladders"),
        ...
        const [ladders, matches, challenges, requests] = await Promise.all([
          laddersRes.json(),
          matchesRes.json(),
          challengesRes.json(),
          requestsRes.json(),
        ...
        setStats({
          ...
          totalMembers: 0, // TODO: aggregate from memberships
